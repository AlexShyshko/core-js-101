...
function getPolynom(...args) {
  ...
  const argumentsArray = [0, 0, 0];
  argumentsArray.splice(3 - argsLength, argsLength, ...args);
  return (x) => argumentsArray[0] * x ** 2 + argumentsArray[1] * x + argumentsArray[2];
}
...
function logger(func, logFunc) {
  ...
  return (...x) => {
    ...
    startStr = `${funcName}(${startArgs.join(',')}) starts`;
    logFunc(startStr);
    const result = func(...x);
    endArgs = x.map((el) => {
      if (Array.isArray(el)) {
    ...
    endStr = `${funcName}(${endArgs.join(',')}) ends`;
    logFunc(endStr);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  const partialFunction = fn;
  const firstPartOfArgs = args1;
  return (...secondPartOfArgs) => partialFunction(...firstPartOfArgs, ...secondPartOfArgs);
}
...
function getIdGeneratorFunction(startFrom) {
  let startNumber = startFrom;
  return () => {
    const prev = startNumber;
    startNumber += 1;
    return prev;
  };
}
